...
%-------------------------------------------------------------------------       
    ...
    % function to store gas fault fluxes
    function state = storefaultfluxes(model, state, qg)
        leakfluxes = {value(qg)};           
        state.fltflux = zeros(model.G.cells.num,1);         
        state = model.setPhaseData(state, leakfluxes, 'fltflux');
    end

    % function to store water fault fluxes
    function state = storefaultfluxeswater(model, state, qw)
        leakfluxesw = {value(qw)};           
        state.fltfluxw = zeros(model.G.cells.num,1);         
        state = model.setPhaseData(state, leakfluxesw, 'fltfluxw');
    end
