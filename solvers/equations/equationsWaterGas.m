...
%}
    ...
    % Hari's Modification for Region Support
    ...
    pcWG = 0;
    if isfield(f, 'pcWG')
        cappress = model.getProps(satstate, 'CapillaryPressure');
        pcWG = deal(cappress{2});
    end
    ...
    eqs{2} = (s.pv/dt) .* (pvMult .* bG .* sG     - pvMult0 .* bG0 .* sG0    ) + s.Div(fluxG) + ff .* sG;
